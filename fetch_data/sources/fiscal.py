...
class FiscalDataFetcher(BaseFetcher):
    ...
    def _handle_average_maturity(self, config):
        ...
        try:
            ...
            # Tables created before the index existed: every run recomputes the
            # summaries for all record dates, and without this index each
            # per-date query scans the whole table (~5 min for 150k rows).
            cursor = conn.cursor()
            try:
                cursor.execute("""
                    ALTER TABLE treasury_average_maturity
